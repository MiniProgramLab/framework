...
function hasComponents(config: NativeConfig | undefined) {
  return (
...
export async function validateCustomTabBar(
  appConfig: NativeConfig,
  { source, pages, pageConfigs, environment, packageEntry }: Parameters<PlatformAdapter['validateTabs']>[1],
) {
  ...
  for (const item of tabBar.list) {
    ...
    if (
      ...
      !hasComponents(
        pageConfigs.get(path.join(source, item.pagePath)),
      )
    ) {
      throw new Error(
  ...
  for (const extension of ['.ts', '.config.ts', '.wxml']) {
    if (!(await exists(entry + extension)))
      throw new Error('自定义 Tabbar 缺少固定入口：' + entry + extension)
  }
  if (!(await Promise.all(['.scss', '.less', '.wxss'].map((suffix) => exists(entry + suffix)))).some(Boolean))
    throw new Error('自定义 Tabbar 缺少样式：' + entry)
