# @skyline-kit/components

基于 Skyline + glass-easel 的原生组件包，包含页面容器、页头、遮罩、弹窗、动作菜单和弹动 Tabbar。组件通过 peerDependency 使用消费项目的 `@skyline-kit/framework`。

## 普通组件

```ts
import { libraryComponents } from '@skyline-kit/components'
...
export default { customTabBar: '@skyline-kit/components/custom-tab-bar/index' }
```
...
`miniprogram` 指向包内 `src`。TS、WXML、SCSS 和 Worklet 由专用 CLI 统一处理，不直接交给微信“构建 npm”。样式 token 可通过 `pkg:@skyline-kit/components/styles/_tokens.scss` 使用；页面与弹层默认视觉保持当前应用表现。
